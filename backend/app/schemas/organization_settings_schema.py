...
class OrganizationSettingsConfig(BaseModel):
    ...
    query_timeout_seconds: FeatureConfig = FeatureConfig(value=180, name="Query progress mark (seconds)", description="How long a single query runs before the agent reports it as still running. This is a progress mark, not a kill: the query keeps going. A connection's config can override this with its own 'query_timeout_seconds' value.", is_lab=False, editable=True)
    query_hard_timeout_seconds: FeatureConfig = FeatureConfig(value=900, name="Query hard limit (seconds)", description="The outer limit for a single query. Only this ends one. Raise it if your warehouse routinely takes longer; lower it to be told sooner. A connection's config may tighten it but never extend it.", is_lab=False, editable=True)
