...
async def _require_instruction_authority(
    ...
    *, allow_owner: bool = False,
    ...
    if ds_ids:
        # ★Fork tier, absent upstream: the AUTHOR of a PRIVATE instruction may
        # act on their own note without holding manage_instructions on the
        # agents it is attached to. A private note is not org governance — it is
        # visible to nobody else — so gating it on authority over the agent
        # would make a member unable to edit their own note. Upstream has no
        # private tier, so its helper checks unconditionally here.
        if owns_private_instruction(existing, current_user):
            return
...
async def update_instruction(
    ...
    if instruction.data_source_ids is not None:
        if instruction.data_source_ids:
            if owns_private_instruction(existing, current_user):
                # Editing your own private note does not carry the right to
                # attach it to an agent you cannot even see, so newly attached
                # agents are still checked — on `view`, not manage_instructions,
                # because nothing shared is being written.
