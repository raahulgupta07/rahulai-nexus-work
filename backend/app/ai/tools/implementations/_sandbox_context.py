...
• **Pre-built UI components** — all global, prefer these for speed but build custom components when the design requires it:
  ...
  - `<BowKpi title="" value="" subtitle="" color="#3B82F6" viz={visualizations[N]} className="" style={{}} />` — **the metric tile. Use it for EVERY KPI / stat / big-number tile.** The value is fitted to the card's real width (measured, then the type is scaled down, and it wraps rather than shrink past legibility), the tile is `min-w-0` so it shrinks inside a grid, and digits are tabular. className adds to defaults (bg-white, border, text-slate-900). Use style={{}} for reliable overrides (e.g. style={{ backgroundColor: '#1e293b', color: '#fff' }}). `maxRem`/`minRem` tune the type range (defaults 1.875 / 0.8125 rem).
    - ⚠️ **NEVER hand-roll a metric tile as `<div className="text-3xl">{value}</div>`.** A fixed type size in a fixed-width card cuts the last digits off a long value with no error and no visual sign — the user reads a wrong number. This is a shipped defect, not a style preference.
    - If your design genuinely needs custom tile markup, still render the VALUE through `<BowFitText className="font-semibold" maxRem={1.875}>{value}</BowFitText>` (global, same fitting logic, no card chrome) inside a `min-w-0` parent.
  - `<KPICard ... />` — alias of `<BowKpi>`, kept for existing dashboards. Prefer `<BowKpi>` in new code.
...
• **PER-ITEM INFO ON CUSTOM MARKUP (`data-dash-*` attributes)** — IMPORTANT for custom dashboards.
  ...
  - `fmt(n, opts)` — number formatter. `{pct: true}` for percentages, no opts for auto K/M/B/T. NEVER print a currency symbol or unit that the data did not give you: pass `{currency: "<ISO-4217 code>"}` ONLY when the data, the column metadata or the instructions state which currency the figures are in. `{currency: true}` names no currency and renders a plain number — that is the correct output for an unknown unit. A wrong unit is worse than none.
...
SANDBOX_RUNTIME_OBSERVATION = (
    ...
    "Pre-built globals (prefer for speed, but build custom React components when the design requires it): LoadingSpinner, "
    ...
    "BowKpi — the metric tile, used for every KPI/stat/big-number: it measures the card and scales the value's type down (then wraps) so a long number is never clipped, is min-w-0, and uses tabular numerals; never hand-roll a tile as a fixed text-3xl div, and when custom markup is unavoidable render the value through BowFitText. KPICard is an alias of BowKpi (className additive, style prop for overrides), SectionCard (className additive, style prop for overrides), FilterSelect (className replaces default theme 'bg-white border-slate-200 text-slate-900' — pass e.g. 'bg-slate-900 border-slate-700 text-slate-100' for dark, portaled dropdown, built-in search at 8+ options), FilterSearch (className replaces default theme), FilterDateRange (className replaces default theme), fmt(), exportCSV(rows, {columns, filename}) for client-side CSV download — wire to an onClick when the user asks for an export button (the ⓘ popover Data tab and DataTable already include one built in). "
