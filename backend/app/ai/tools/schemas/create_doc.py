...
class CreateDocInput(BaseModel):
    ...
    markdown: str = Field(..., description=(
        ...
        "- Images: embed one with {{file:<file_id>}} on its own line, or "
        "{{file:<file_id>|caption}} to caption it. Use file ids from generate_image results or "
        "uploaded image files. Only images render; a file the organization doesn't own is dropped.\n"
