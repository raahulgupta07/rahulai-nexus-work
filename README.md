...
**Current version: `0.0.543.7`** — see [CHANGELOG.md](CHANGELOG.md) for what shipped, and [UPGRADE.md](UPGRADE.md) to install or upgrade.
...
dash-app                         project=cityagentinsights  image=cityagentinsights:0.0.543.3
...
#64 naming to docker.io/library/cityagentinsights:0.0.543.3 done
#64 DONE 72.6s
 Image cityagentinsights:0.0.543.3 Built
```
...
```
health 200
{"current_version":"0.0.543.3","available":true,"versions":[{"version":"0.0.543.3","date
...
docker tag cityagentinsights:pre-0.0.543.3 "$IMG"  # use YOUR version here
docker compose $COMPOSE up -d app
...
docker run --rm --entrypoint sh cityagentinsights:pre-0.0.543.3 \
